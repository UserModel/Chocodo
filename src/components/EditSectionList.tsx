...
import { setCurrentGame, addNewGame, editSection, deleteAllTasksFromSection, deleteSection } from '../slices/userSlice'
...
export const EditSectionList = (props: PropTypes) => {
    ...
    const editSectionName = (section: Section) => {
        ...
        if ( editedSectionName ) {
            dispatch(editSection(game.id, {...section, sectionName: editedSectionName}))
                        ...
                        { isEditing !== section.id ? (
                            <Flex w="100%">
                                ...
                                <Flex marginLeft="1%" h="100%">
                                    <IconButton size="xs" aria-label="edit-button" icon={<EditIcon color={iconColor} bgColor={iconBg} />} onClick={() => setIsEditing(section.id)} />
                                    <IconButton size="xs" marginLeft="5%" aria-label="delete-button" onClick={() => removeSection(section)} icon={<DeleteIcon color={iconColor} bgColor={iconBg} />} />
                                </Flex>
                            </Flex>
                        ) : (
                            <>
                                <Input 
                                    padding="5px"
                                    autoFocus
                                    onFocus={(e) => setEditedSectionName(section.sectionName)}
                                ...
                                />
                                <Flex marginLeft="1%" h="100%">
                                    <IconButton size="xs" aria-label="edit-submit-button" icon={<CheckIcon color={iconColor} bgColor={iconBg} />} onClick={() => editSectionName(section)} />
                                    <IconButton size="xs" marginLeft="5%" aria-label="edit-cancel-button" onClick={() => {setEditedSectionName(""); setIsEditing(0);}} icon={<CloseIcon color={iconColor} bgColor={iconBg} />} />
                                </Flex>
                            </>
                        ) }
            ...
            <ModalContent h="auto" maxH="75%" >
                ...
                <ModalBody overflowY="auto">
                    <VStack spacing={4} paddingBottom="5%">
                        ...
                        <Box bgColor={bgColor} w="400px" minH="auto" maxH="400px" overflowY="auto" border="2px">
                        ...
                        { game.hasWeekly && 
                            <>
                                <Heading size="sm">Weekly Task Sections</Heading>
                                <Box bgColor={bgColor} w="400px" minH="auto" maxH="400px" overflowY="auto" border="2px">
